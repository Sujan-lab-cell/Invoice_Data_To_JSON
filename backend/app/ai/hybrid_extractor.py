...
class HybridInvoiceExtractor:
    ...
    def _has_missing_data(self, invoice: Invoice) -> bool:
        ...
        is empty or missing after rule-based extraction.
        """
        # 1. Critical headers
        if not invoice.invoice_number or not str(invoice.invoice_number.raw).strip():
            return True
        if not invoice.invoice_date or not str(invoice.invoice_date.raw).strip():
            return True

        # 2. Supplier & Buyer details
        if not invoice.supplier or not invoice.supplier.name or not str(invoice.supplier.name.raw).strip():
            return True
        if not invoice.buyer or not invoice.buyer.name or not str(invoice.buyer.name.raw).strip():
            return True
        ...
        for item in invoice.items:
            ...
            if (not pricing or
                not pricing.mrp or pricing.mrp.normalized is None or pricing.mrp.normalized == 0.0 or
                not pricing.purchase_rate or pricing.purchase_rate.normalized is None or pricing.purchase_rate.normalized == 0.0 or
                not pricing.taxable_amount or pricing.taxable_amount.normalized is None or pricing.taxable_amount.normalized == 0.0):
                return True

        # 4. Totals
        if not invoice.totals or not invoice.totals.grand_total or invoice.totals.grand_total.normalized is None or invoice.totals.grand_total.normalized == 0.0:
            return True
    ...
    def _merge_gemini_extraction(self, invoice: Invoice, gemini_data: Dict[str, Any]) -> Invoice:
        ...
        if isinstance(gemini_items, list) and len(gemini_items) > 0:
            ...
            # Use Gemini's items if rule items are empty or lack pricing details
            if not invoice.items or len(invoice.items) == 0:
                invoice.items = validated_items
            else:
                has_incomplete_rule_items = any(
                    not item.pricing or not item.pricing.mrp or item.pricing.mrp.normalized == 0.0
                    for item in invoice.items
                )
                if has_incomplete_rule_items and len(validated_items) >= len(invoice.items):
                    invoice.items = validated_items

        # 5. Totals Recalculation & Merge
        if invoice.items and len(invoice.items) > 0:
            ...
            if not invoice.totals:
                invoice.totals = Totals(
                    subtotal=ValuePair(raw=f"{subtotal_val:.2f}", normalized=subtotal_val, confidence=1.0),
                    tax_total=ValuePair(raw=f"{tax_val:.2f}", normalized=tax_val, confidence=1.0),
                    grand_total=ValuePair(raw=f"{grand_val:.2f}", normalized=grand_val, confidence=1.0)
                )
            else:
